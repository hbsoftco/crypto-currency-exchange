		...
		<div class="w-full overflow-y-scroll">
			<table class="min-w-full py-6 text-right">
				...
				<tbody>
					...
					<template v-else>
						<tr
							v-for="(item, index) in InternalReceiveList"
							:key="index"
						...
						>
							<td class="text-nowrap text-xs font-normal py-2">
								{{ useNumber(formatDateToIranTime(item.doneTime)) }}
							</td>
							<td class="text-nowrap text-xs font-normal py-2">
								{{ useNumber(item.userFrom) }}
							</td>
							<td class="text-nowrap text-xs font-normal py-2">
								{{ useNumber(item.userTo) }}
							</td>
							<td class="text-nowrap text-xs font-normal py-2">
								<div class="flex">
									<NuxtImg
...
const params = ref<GetInternalReceiveParams>({
	currencyId: '',
	source: '',
	destination: '',
	from: '',
...
const applyFilters = async () => {
	params.value.source = source.value;
	params.value.destination = destination.value;
	params.value.from = fromDate.value;
	params.value.to = toDate.value;

	await loadInternalReceive();
};
