<template>
	...
	>
		...
		<div
			...
			class="my-4"
		>
			<UiTitleWithBack :title="$t('deleteAccount')" />
		</div>
		<section>
			<div class="grid grid-cols-1 md:grid-cols-2 gap-20 rounded-none md:rounded-md mt-0 md:mt-8 mb-0 md:mb-4 py-6 md:py-10 px-4 md:px-20 bg-primary-gray-light dark:bg-primary-gray-dark">
				...
				<div class="my-0 md:my-8">
					<SideGuideBox :tag-type="TagType.ACCOUNT_DELETE" />
				</div>
			</div>
		</section>
	</div>
...
const reasonListParams = ref<ReasonListParams>({
	entityType: String(RejectReason.AccountDeleteReason),
...
const submit = async () => {
	...
	try {
		const confirmation = await $swal.fire({
			title: useT('deleteAccount'),
			text: useT('deleteAccountDescription'),
			icon: 'warning',
